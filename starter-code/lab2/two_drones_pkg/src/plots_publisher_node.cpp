...
class PlotsPublisherNode : public rclcpp::Node {
  ...
  std::shared_ptr<tf2_ros::TransformListener> tf_listener_{nullptr}; // Listens to tf
  std::unique_ptr<tf2_ros::Buffer> tf_buffer_; // stores/caches the transforms
  ...
  class TrajTrail {
    ...
    void update() {
      ...
      try {
        ...
        // canTransform asks : Can tf give me the pose of dest_frame expressed in ref_frame?
        // parent->tf_buffer_ means: from parent node get tf buffer - call canTransform object on the buffer object
        if (parent->tf_buffer_->canTransform(
          ...
          // lookupTranform means: Give me the pose of dest_frame expressed in ref_frame
